 <?php
    ...
    if(isset($_POST['login_submit'])){
        ...
        if ((isset($_POST['email']) && !empty($_POST['email'])) && (isset($_POST['password']) && !empty($_POST['password']))) {
            ...
            $resultados = mysqli_query($conn, "select email, password, active from clients where email='$escapedMail' AND active='1';");
            ...
            if (mysqli_num_rows($resultados) > 0 && mysqli_num_rows($resultados2) == 0) {
                ...
                if ($escapedMail === $linha['email']) {
                    ...
                    if (password_verify($escapedPassword, $linha['password'])) {
                        $resultados = mysqli_query($conn, "select * from clients where email='$escapedMail' AND active='1';");
                        $linha = mysqli_fetch_assoc($resultados);
