...
if (isset($_POST['checkout_submit'])) {
    ...
    $searchStock = mysqli_query($conn, "select * from produtos, compra, albums where compra.id=produtos.compra_id and albums.id=produtos.albums_id and compra.clients_id='".$_SESSION['user_id']."' and compra.finalizado='0'");
    ...
    if ($userBalance >= $cartTotal) {
        ...
        while($linha = mysqli_fetch_assoc($searchStock)){
            $currentStock = $linha['stock'];
            $updatedStock = $currentStock - $linha['qtd'];
            mysqli_query($conn,"UPDATE albums SET stock ='".$updatedStock."' where id='" . $linha['id'] . "'") or die("Error: " . mysqli_error($conn));
        }
